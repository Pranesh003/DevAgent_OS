...
function CodeBlock({ file }: { file: any }) {
  const { selectedFile, setSelectedFile } = useAgentStore();
  const isSelected = selectedFile?.path === (file.path || file.filename);
  
  return (
    <button 
      onClick={() => setSelectedFile({ ...file, path: file.path || file.filename, type: 'code' })}
      style={{
        ...
        borderRadius: 8, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8,
        transition: 'all 0.2s'
      }}
      className="hover:border-[var(--accent-primary)] hover:bg-[var(--bg-surface-hover)]"
    >
      <span style={{ fontSize: 14 }}>📄</span>
      <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
        <span style={{ fontSize: 13, color: isSelected ? 'var(--accent-primary)' : 'var(--text-primary)', fontWeight: isSelected ? 600 : 500, fontFamily: 'var(--font-mono)', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }}>
          {file.path || file.filename}
        </span>
      </div>
      <span className="badge badge-idle" style={{ fontSize: 10, padding: '2px 8px' }}>{file.language}</span>
    </button>
  );
}

/* ── Clickable JSON/Architecture detail block ─────────────────────────── */
function DetailBlock({ title, icon, data, type }: { title: string; icon: string; data: any; type: 'architecture' | 'debug' | 'testing' }) {
  const { selectedFile, setSelectedFile } = useAgentStore();
  const isSelected = selectedFile?.path === title;
  ...
  return (
    <button 
      onClick={() => setSelectedFile({ path: title, content: text, type, language: 'json' })}
      style={{
        width: '100%', textAlign: 'left', padding: '10px 14px', 
        background: isSelected ? 'var(--bg-surface-hover)' : 'var(--bg-surface)', 
        border: `1px solid ${isSelected ? 'var(--accent-primary)' : 'var(--bg-border)'}`, 
    ...
    >
      ...
      <span style={{ fontSize: 13, color: isSelected ? 'var(--accent-primary)' : 'var(--text-secondary)', fontWeight: isSelected ? 600 : 500 }}>{title}</span>
    </button>
  );
}
...
function DocBlock({ filename, content }: { filename: string; content: string }) {
  const { selectedFile, setSelectedFile } = useAgentStore();
  const isSelected = selectedFile?.path === filename;

  return (
    <button 
      onClick={() => setSelectedFile({ path: filename, content, type: 'documentation', language: 'markdown' })}
      style={{
        width: '100%', textAlign: 'left', padding: '10px 14px', 
        background: isSelected ? 'var(--bg-surface-hover)' : 'var(--bg-surface)', 
        border: `1px solid ${isSelected ? 'var(--accent-primary)' : 'var(--bg-border)'}`, 
    ...
    >
      <span style={{ fontSize: 14 }}>📝</span>
      <span style={{ fontSize: 13, color: isSelected ? 'var(--accent-primary)' : 'var(--text-accent)', fontWeight: isSelected ? 600 : 500, fontFamily: 'var(--font-mono)' }}>
        {filename}
      </span>
    </button>
  );
...
function AgentOutputSection({ agent }: { agent: string }) {
  ...
  if (agent === 'ArchitectureAgent' && architecture && Object.keys(architecture).length > 0) {
    return (
      <div style={{ padding: '0 16px 12px', marginTop: -4 }}>
        <DetailBlock title="System Architecture" icon="🏛️" data={architecture} type="architecture" />
      </div>
    );
  }
  ...
  if (agent === 'DebugAgent' && debugOutput && Object.keys(debugOutput).length > 0) {
    return (
      <div style={{ padding: '0 16px 12px', marginTop: -4 }}>
        <DetailBlock title="Debug Analysis" icon="🐛" data={debugOutput} type="debug" />
      </div>
    );
  }
  ...
  if (agent === 'TestingAgent' && tests && tests.length > 0) {
    return (
      <div style={{ padding: '0 16px 12px', marginTop: -4 }}>
        <DetailBlock title={`Test Cases (${tests.length})`} icon="🧪" data={tests} type="testing" />
      </div>
    );
  }
...
export default function UnifiedAgentFeed() {
  const { messages, status, currentPhase, critiqueScore, codeFiles, documentation } = useAgentStore();
