...
const UnifiedAgentFeed = React.lazy(() => import('@/components/agents/UnifiedAgentFeed'));
const CodePreviewPanel = React.lazy(() => import('@/components/code/CodePreviewPanel'));
...
export default function WorkspacePage() {
  ...
  const { sessionId, status, startSession, addMessage, setStatus, setCodeFiles, setDocumentation, setArchitecture, setDebugOutput, setTests } = useAgentStore();
      ...
      {status !== 'idle' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(400px, 1fr) minmax(500px, 1.2fr)', gap: 24, alignItems: 'start' }}>
            <Suspense fallback={<div style={{ padding: 20, textAlign: 'center' }}>Loading Feed...</div>}>
              <UnifiedAgentFeed />
            </Suspense>
            <Suspense fallback={<div style={{ padding: 20, textAlign: 'center' }}>Loading Preview...</div>}>
